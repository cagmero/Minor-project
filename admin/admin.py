from flask import Blueprint, request, render_template, abort, session, redirect, url_for, current_app
...
def check_excel_sheet_subject():
    category_directory = os.path.join(current_app.root_path,"static/db_format",session['data_master_mode'])
    ...
    if len(request.files) != 0:
        ...
        if up_rec_excel_filename!='':
            if data_master_mode == "student" or data_master_mode == "subject" or data_master_mode == "electives_category" or data_master_mode == "electives_subject":
                ...
                # print("mode",mode)
                excel_directory = os.path.join(current_app.root_path,"static/spreadsheets",dept,data_master_mode,mode)
            elif data_master_mode == "faculty":
                excel_directory = os.path.join(current_app.root_path,"static/spreadsheets",dept,data_master_mode)
            ...
            if data_master_mode == "student":
                ...
                for r in range(1, sheet.nrows):
                    ...
                    if cursor.rowcount == 1:
                        img_directory = os.path.join(current_app.root_path,"static/profile_pics/student_images",dept,mode,str('semester_'+str(sem)),str(stud_id))
                        print(img_directory)
                ...
                for r in range(1, sheet.nrows):
                    ...
                    cursor.execute(query, values)
                    if cursor.rowcount == 1:
                        img_directory = os.path.join(current_app.root_path,"static/profile_pics/faculty_images",dept,str(fac_id))
                        print(img_directory)
                        Path(img_directory).mkdir(parents=True, exist_ok=True)
                        for f in os.listdir(img_directory):
                            os.remove(os.path.join(img_directory, f))
...
def upload_excel_export_records():
    ...
    if session.get('dept') is not None:
        dept = session['dept']
    if data_master_mode != "":
        download_directory = os.path.join(current_app.root_path,"static/spreadsheets/Download")
        ...
        if data_master_mode == "student":
            ...
            if mode != "" :
                ...
                row_insert = 1
                # img_directory = os.path.join(current_app.root_path,"static/profile_pics/student_images",dept,mode)
                # img_directory = os.path.join(current_app.root_path,"static/profile_pics/student_images",dept,mode,str('semester_'+str(sem)),str(stud_id))
                for row in records:
                    print(row)
...
def insert_student_entry():
    ...
    if request.files['s_photo'].filename != '':
        ...
        directory = os.path.join(current_app.root_path,"static/profile_pics/student_images",dept,mode,str('semester_'+sem),stud_id)
        # s_photo_file = os.path.join(directory,s_photo)
        print(directory)
...
def insert_faculty_entry():
    ...
    if request.files['f_photo'].filename != '':
        ...
        directory = os.path.join(current_app.root_path,"static/profile_pics/faculty_images",session['dept'],fac_id)
        # s_photo_file = os.path.join(directory,s_photo)
        print(directory)
        Path(directory).mkdir(parents=True, exist_ok=True)
        for f in os.listdir(directory):
...
def update_faculty_entry():
    ...
    if len(request.files) != 0:
        ...
        print(f_photo_name)
        if f_photo_name != '':
            directory = os.path.join(current_app.root_path,"static/profile_pics/faculty_images",dept,fac_id)
            # s_photo_file = os.path.join(directory,s_photo)
            print(directory)
            Path(directory).mkdir(parents=True, exist_ok=True)
            for f in os.listdir(directory):
