...
def dashboard():
    ...
    if mode=="Faculty":
        ...
        # Add actual quizzes for Admin to monitor
        cursor.execute('SELECT quiz_id,q_title,q_sub,q_date,q_time_start,q_time_end,quiz_type FROM quiz_det ORDER BY quiz_id DESC')
        records = cursor.fetchall()
        for row in records:
            row['q_date'] = datetime.datetime.strptime(row['q_date'], '%Y-%m-%d').strftime('%d/%m/%y')
            qs.append(row)
