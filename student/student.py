from flask import Blueprint, request, render_template, abort, session, redirect, url_for,jsonify, current_app
            ...
            if rec is None:
                if session.get('submitted_ques') is None or session.get('q_nos') is None:
                    if user_attempt_ques is None:
                        ...
                        session['quiz_start'] = now.strftime('%H:%M:%S')
                        # Set demo timer to 2 minutes
                        session['quiz_end'] = (now + datetime.timedelta(minutes=2)).strftime('%H:%M:%S')
                        session['quiz_date'] = now.strftime('%Y-%m-%d')
        ...
        if request.method == "POST":
            ...
            if ans_type==0:
                ...
                if len(request.files) != 0:
                    ...
                    directory = os.path.join(current_app.root_path,"student/descriptive_ans_uploads",session['dept'],str(session['sem']),str(session['quiz_id']),str(session['svv']),str(session['current_ques']))
...
def question_submit():
    if session.get('submitted_ques') is not None or session.get('q_nos') is not None:
        ...
        if ans_type==0:
            ...
            desc_ans_name = 'TEXT' # Placeholder to indicate text answer
            desc_ans_file = request.form.get('desc_ans', '') # Get text from textarea
...
def finish_quiz():
    ...
    try:
        ...
        ttl_time_taken = datetime.datetime.strptime("00:00:00", '%H:%M:%S')
    # #
